 ...
 <div class="wide"><small>Concepto</small><strong>${esc(p.concept)}</strong></div><div class="wide"><small>Autorización de Gerencia</small><strong class="${authorized ? "approval-ok" : "approval-pending"}">${authorized ? `Pago autorizado por Gerencia · ${esc(bogotaDateTime(p.authorized_at))}` : "Pendiente de autorización de Oscar Pacheco"}</strong></div>
  ...
  async function paymentReport() {
    ...
    report.document.write(
      `<!doctype html><html lang="es"><head><meta charset="utf-8"><title>${esc(reportId)} · Orden de pagos Creditek</title><style>@page{size:A4 landscape;margin:12mm}*{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact}body{font-family:Montserrat,Arial,sans-serif;color:#0B1E3D;margin:0}.head{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:4px solid #00C4CC;padding-bottom:12px}.brand{display:flex;align-items:center;gap:18px}.head img{width:155px;max-height:60px;object-fit:contain}.head h1{margin:0;font-size:23px}.eyebrow{font-size:10px;letter-spacing:.13em;text-transform:uppercase;color:#4b6b7e;margin-bottom:5px}.meta{font-size:11px;color:#536176;line-height:1.55;text-align:right}.meta strong{color:#0B1E3D}.summary{display:flex;gap:12px;margin:14px 0}.pill{background:#eefbfd;border:1px solid #00C4CC;border-radius:9px;padding:8px 13px;min-width:135px;font-size:11px;text-transform:uppercase;letter-spacing:.05em}.pill strong{display:block;font-size:20px;margin-top:3px;letter-spacing:0;text-transform:none}table{width:100%;table-layout:fixed;border-collapse:collapse;font-size:11.5px;line-height:1.4}thead{display:table-header-group}th{background:#0B1E3D;color:#fff;padding:8px 6px;text-align:left;font-size:10.5px;overflow-wrap:anywhere}td{padding:9px 6px;border-bottom:1px solid #dfe5ec;vertical-align:top;overflow-wrap:anywhere}tr{break-inside:avoid}.money{text-align:right;font-size:12px;font-weight:700;white-space:nowrap;font-variant-numeric:tabular-nums}.payment-number{font-size:13px;font-weight:700}.account{font-size:12px;font-weight:600;font-variant-numeric:tabular-nums}.internal-ref{display:block;color:#64748b;font-size:8px;line-height:1.4;margin-top:3px}.muted{color:#536176;font-size:11px}.total td{font-size:14px;font-weight:800;border-top:3px solid #00C4CC;background:#eefbfd}.trace{margin-top:14px;padding:10px 12px;border:1px solid #d8e2ea;border-radius:8px;background:#f8fafc;font-size:10.5px;line-height:1.5}.trace strong{color:#0B1E3D}.foot{display:flex;justify-content:space-between;gap:20px;margin-top:12px;padding-top:9px;border-top:1px solid #ccd6e0;font-size:10px;color:#536176}.tools{position:fixed;right:18px;bottom:18px;display:flex;align-items:center;gap:10px}.print-note{background:#fff7df;border:1px solid #e6b84a;border-radius:8px;padding:9px 12px;font-size:11px;max-width:330px}.no-print button{background:#00C4CC;color:#0B1E3D;border:0;border-radius:10px;padding:12px 18px;font-weight:800}@media print{.no-print{display:none!important}}</style></head><body><header class="head"><div class="brand"><img src="${esc(logo)}" alt="Creditek"><div><div class="eyebrow">Tesorería · Documento operativo</div><h1>Orden de pagos autorizados</h1></div></div><div class="meta"><strong>${esc(reportId)}</strong><br>Generado: ${esc(generated)}<br>Responsable: ${esc(profile?.nombre || profile?.email || "Usuario KORA")}<br>Estado: autorizado · pendiente de pago y soporte<br>Incluye todos los pagos autorizados; no aplica filtros de consulta</div></header><section class="summary"><div class="pill">Pagos incluidos<strong>${rows.length}</strong></div><div class="pill">Valor total<strong>${cop(total)}</strong></div><div class="pill">Liquidaciones<strong>${liquidationRefs.length}</strong></div></section><table><colgroup><col style="width:5.5%"><col style="width:17%"><col style="width:11%"><col style="width:11%"><col style="width:15%"><col style="width:12%"><col style="width:15%"><col style="width:13.5%"></colgroup><thead><tr><th>N.º</th><th>Negocio / titular</th><th>Identificación</th><th>Banco / tipo</th><th>Número de cuenta</th><th>Origen / fecha</th><th>Concepto</th><th style="text-align:right">Valor</th></tr></thead><tbody>${rows.map((p, i) => `<tr><td><span class="payment-number">${i + 1}</span><span class="internal-ref" title="${esc(p.report_ref)}">Ref.<br>${esc(String(p.report_ref || "").slice(-4).toUpperCase())}</span></td><td><strong>${esc(p.report_business || paymentBusinessName(p) || "No aplica")}</strong><br><span class="muted">Titular: ${esc(p.bank_snapshot.holder || p.beneficiary_name)}</span></td><td>${esc(p.bank_snapshot.holder_identification || p.beneficiary_identification)}</td><td>${esc(p.bank_snapshot.bank)}<br><span class="muted">${esc(p.bank_snapshot.account_type)}</span></td><td class="account">${esc(p.bank_snapshot.account_number)}</td><td>${p.liquidation_id ? `${esc(platformName(p.platform_snapshot))}<br><span class="muted">Corte ${date(p.cutoff_snapshot)}</span>` : `${esc(p.report_kind)}<br><span class="muted">${esc(p.report_date)}</span>`}</td><td>${esc(p.concept)}</td><td class="money">${cop(p.valor)}</td></tr>`).join("")}<tr class="total"><td colspan="7">TOTAL A GIRAR</td><td class="money">${cop(total)}</td></tr></tbody></table><section class="trace">Cada pago debe registrarse en KORA con su soporte antes de marcarse como pagado y posteriormente conciliado. La referencia corta es interna; no es un comprobante bancario.<details class="no-print"><summary>Trazabilidad KORA · referencias completas</summary>${rows.map((p, i) => `<div>${i + 1}: ${esc(p.report_ref)}${p.liquidation_id ? ` · LQ-${shortId(p.liquidation_id)}` : ""}</div>`).join("")}</details></section><footer class="foot"><div><strong>Creditek S.A.S. · NIT 901.259.859-0</strong><br>Documento generado electrónicamente por KORA.</div><div style="text-align:right">Valores en pesos colombianos (COP)<br>La numeración indica el orden de los pagos en este documento.</div></footer><div class="tools no-print"><div class="print-note">Para un PDF limpio, en <strong>Más ajustes</strong> desactiva “Encabezados y pies de página”.</div><button onclick="window.print()">Imprimir / Guardar PDF</button></div></body></html>`,
    );
