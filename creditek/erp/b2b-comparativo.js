(function(root){
 ...
 const O=typeof module==='object'&&module.exports?require('./b2b-oportunidades.js'):root.KoraB2BOportunidades;
 const R=typeof module==='object'&&module.exports?require('./b2b-referencias-catalogo.js'):root.KoraB2BReferencias;
 ...
 const order=(a,b)=>a.costo-b.costo||a.precio_tienda-b.precio_tienda||String(a.proveedor_id).localeCompare(String(b.proveedor_id))||String(a.id||a.row||'').localeCompare(String(b.id||b.row||''));
 ...
 function build({providers,products,offers,winners,drafts,references=[],generatedAt=new Date().toISOString()}){
  const grouping=R.create(references),canonicalProducts=new Map(grouping.unique(products).map(p=>[p.id,p]));
  ...
  const selected=new Map(winners.map(o=>[grouping.id(o.producto_id),{...grouping.offer(o),costo:amount(o.costo),precio_tienda:amount(o.precio_tienda)}]));
  ...
  for(const draft of latestDrafts(drafts)){
   ...
   for(const row of draft.filas){
    ...
    else draftOffers.push({...row,costo:cost,precio_tienda:price,sourceDate:draft.creado_at,draftId:draft.id});
   }
  }
  function matrix(rows,isPublished){
   const grouped=new Map();
   for(const source of rows){const row=grouping.offer(source);if(!grouped.has(row.producto_id))grouped.set(row.producto_id,[]);grouped.get(row.producto_id).push(row);}
   return [...grouped].map(([id,values])=>{
    // Un proveedor puede ofrecer varios colores: mostramos su menor costo,
    // conservando todas las ofertas originales y la oferta ganadora para auditoría.
    const cells=suppliers.map(p=>values.filter(v=>v.proveedor_id===p.id).sort(order).slice(0,1));
    const candidates=cells.flat(),best=[...candidates].sort(order)[0],actual=selected.get(id),chosen=isPublished?actual:best;
    const tied=candidates.filter(v=>v.costo===best.costo);
    const matches=!!actual&&actual.proveedor_id===best.proveedor_id&&actual.costo===best.costo&&actual.precio_tienda===best.precio_tienda&&(!actual.id||actual.id===best.id);
    return {product:canonicalProducts.get(id),cells,sourceOffers:values,best,chosen,actual,ties:tied.length,status:isPublished?(matches?'Coincide con el menor costo vigente':'REVISAR: selección publicada distinta o no disponible'):'Propuesta parcial · no publicada'};
 ...
 async function load(sb,profile){
  ...
  const [providers,products,offers,winners,metadata,references]=await Promise.all([
   ...
   all(()=>sb.from('b2b_mejor_oferta').select('id,producto_id,proveedor_id,costo,precio_tienda').order('producto_id')),
   all(()=>sb.from('b2b_catalogo_borradores').select('id,proveedor_id,creado_at,lista_id').order('creado_at',{ascending:false}).order('id')),
   all(()=>sb.from('b2b_referencias_catalogo').select('producto_id,referencia_id,nombre_pedido').order('producto_id'))
  ]);
  ...
  return build({providers,products,offers,winners,drafts,references});
 }
