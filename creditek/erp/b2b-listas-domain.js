(function(root){
 ...
 const marginPolicy='Política retail: costo menor de $150.000 + 12%; desde $150.000 + $20.000';
 const defaultMargin=cost=>Number.isFinite(cost)&&cost>0&&cost<150000?Math.round(cost*12)/100:20000;
 const isDefaultReason=reason=>!String(reason??'').trim()||reason===marginPolicy;
 function amount(v){
 ...
 function prepare(rows,map,products,providers,fixedProvider=''){
  ...
  for(let i=map.header+1;i<rows.length;i++){
   ...
   const margin=hasMargin?amount(rawMargin):defaultMargin(cost);
 ...
 function validate(rows){
  ...
  for(const r of selected){
   ...
   if(Math.round((r.precio_tienda-r.costo)*100)!==defaultMargin(r.costo)*100&&isDefaultReason(r.motivo))errors.push(prefix+'indica el motivo del margen especial.');
 ...
 const api={key,amount,prepare,validate,winners,csv,defaultMargin,marginPolicy,isDefaultReason};
