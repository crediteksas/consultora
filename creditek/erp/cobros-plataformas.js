(function (global, factory) {
  ...
  function create({ sb, money, canEdit = false, canVoid = false, initialMonth = todayBogota().slice(0, 7) } = {}) {
    ...
    function bankConfirmationQueue() {
      ...
      if (!rows.length) return '';
      return `<section class="cobros-bank-queue" aria-label="Liquidaciones pendientes de confirmación bancaria"><h3>Liquidaciones pendientes de confirmación bancaria · ${rows.length}</h3><p class="cobros-muted">Resumen de todos los meses; los indicadores de arriba corresponden al mes seleccionado. Abre el detalle para comparar el importe con el banco y confirmar una sola vez.</p><div class="cobros-bank-grid">${rows.map(row => `<article class="cobros-record"><div class="cobros-record-head"><h4>${esc(platformName(row.plataforma))} · Corte ${esc(row.corte || 'sin fecha')}</h4><span class="cobros-status${row.overdue ? ' cobros-status--alert' : ''}">${row.overdue ? (row.applied ? 'Vencido · abono parcial' : 'Vencido · sin confirmar') : row.applied ? 'Abono parcial' : 'Pendiente de banco'}</span></div><dl class="cobros-record-fields"><div><dt>Debe ingresar al banco</dt><dd>${cash(row.amount)}</dd></div><div><dt>Fecha prevista</dt><dd>${shortDate(row.day)}</dd></div><div><dt>Ya aplicado</dt><dd>${cash(row.applied)}</dd></div><div><dt>Falta por confirmar</dt><dd>${cash(row.remaining)}</dd></div></dl><button type="button" class="btn secondary" data-cobros-action="detail" data-expected-id="${esc(row.id)}">${canEdit ? 'Ver detalle y confirmar' : 'Ver detalle'}</button></article>`).join('')}</div></section>`;
    }
    ...
    function onClick(event) {
      ...
      if (button.dataset.cobrosAction === 'detail') {
        const row = state.data?.expected.find(item => item.id === button.dataset.expectedId && active(item));
        if (!row) return;
        state.month = /^\d{4}-\d{2}-\d{2}$/.test(String(row.corte || '')) ? row.corte.slice(0, 7) : '';
        state.platform = row.plataforma;
        render();
        const cut = [...state.container.querySelectorAll('[data-cobros-cut]')].find(item => item.dataset.cobrosCut === row.id);
        if (cut) { cut.open = true; cut.scrollIntoView?.({ block: 'start' }); }
        return;
      }
