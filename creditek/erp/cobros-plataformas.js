(function (global, factory) {
  ...
  function create({ sb, money, canEdit = false, canVoid = false, initialMonth = todayBogota().slice(0, 7) } = {}) {
    ...
    function expectedCard(row) {
      ...
      return `<article class="cobros-record${!active(row) ? ' cobros-record--void' : ''}"><div class="cobros-record-head"><h4>Corte ${esc(row.corte || 'sin corte')}</h4><span class="cobros-status${row.overdue ? ' cobros-status--alert' : ''}">${status}</span></div><p>${esc(row.concepto || 'Cobro de plataforma')}</p><dl class="cobros-record-fields"><div><dt>Fecha esperada</dt><dd>${shortDate(row.day)}</dd></div><div><dt>Esperado</dt><dd>${cash(row.amount)}</dd></div><div><dt>Aplicado</dt><dd>${cash(row.applied)}</dd></div><div><dt>Pendiente</dt><dd>${active(row) ? cash(row.remaining) : 'No aplica'}</dd></div><div><dt>Fuente</dt><dd>${esc(sourceName(row.fuente_tipo))}${row.liquidation_id ? ` · ${esc(row.liquidation_id)}` : ''}</dd></div><div><dt>Soporte</dt><dd>${support(row.soporte)}</dd></div></dl>${inBankQueue(row) ? '' : confirmReceivedForm(row)}${active(row) ? voidForm('expected', row.id) : `<p class="cobros-muted">${esc(row.motivo_anulacion || 'Anulado; consulta el historial de cambios.')}</p>`}</article>`;
    }
    ...
    function platformCard(item) {
      ...
      const cutRows = cuts.map(row => {
        ...
        return `<details class="cobros-cut" data-cobros-cut="${esc(row.id)}"><summary><span class="cobros-cut-date"><small>Corte</small><strong>${esc(row.corte)}</strong></span><span><small>Esperado</small><strong>${cash(row.amount)}</strong></span><span><small>Recibido aplicado</small><strong>${cash(row.applied)}</strong></span><span><small>Pendiente</small><strong>${cash(row.remaining)}</strong></span><span class="cobros-status ${row.remaining === 0 ? 'cobros-status--ok' : row.overdue ? 'cobros-status--alert' : ''}">${label}</span><span class="cobros-cut-link">Ver detalle</span></summary><div class="cobros-cut-detail">${expectedCard(row)}<h4>Abonos vinculados a este corte</h4>${renderGroup(linked, depositCard, 'No hay abonos aplicados a este corte.')}</div></details>`;
      }).join('');
      const unassigned = deposits.filter(d => active(d) && d.remaining > 0);
      return `<section class="cobros-platform" id="cobros-platform-${esc(item.plataforma)}"><div class="cobros-record-head"><h3>${esc(platformName(item.plataforma))}</h3><span class="cobros-status${item.pending === 0 && item.unapplied === 0 ? ' cobros-status--ok' : ''}">${item.pending === 0 && item.unapplied === 0 ? 'Sin saldos pendientes registrados' : 'Requiere revisión'}</span></div>${metrics(item)}<h4 class="cobros-group-title">Conciliación por corte · ${cuts.length} cortes</h4><p class="cobros-muted">Cada fila reúne el esperado y los abonos aplicados al mismo corte. Abre el detalle para ver la trazabilidad.</p><div class="cobros-cut-list">${cutRows || '<p class="cobros-empty">No hay cobros esperados registrados.</p>'}</div>${unassigned.length ? `<h4 class="cobros-group-title">Abonos con saldo sin aplicar</h4>${unassigned.map(depositCard).join('')}` : ''}<details class="cobros-history"><summary>Consultar todos los abonos (${deposits.filter(active).length})</summary><div class="cobros-cut-detail">${renderGroup(deposits.filter(active).sort((a,b) => b.day.localeCompare(a.day)), depositCard, 'No hay abonos registrados.')}</div></details>${expected.some(row => !active(row)) || deposits.some(row => !active(row)) ? `<details class="cobros-history"><summary>Registros anulados</summary><div class="cobros-columns"><div>${expected.filter(row => !active(row)).map(expectedCard).join('')}</div><div>${deposits.filter(row => !active(row)).map(depositCard).join('')}</div></div></details>` : ''}</section>`;
    }
