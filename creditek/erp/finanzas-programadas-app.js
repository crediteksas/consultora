(function () {
  ...
  function actionButtons(row){if(row.status==='pendiente_aprobacion')return '<a class="btn" href="aliados-tesoreria.html?vista=gastos">Pendiente en Tesorería</a>';if(row.status==='aprobado')return `<button class="btn primary" data-action="payment" data-id="${row.id}">Registrar pago</button>`;return '';}
