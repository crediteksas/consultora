(function () {
  ...
  function renderManualForm(record){const withdrawal=record?.entry_type==='retiro_utilidad';retailRequest=crypto.randomUUID();retailPayload=null;$('modalTitle').textContent=withdrawal?'Nuevo retiro de utilidad':'Nuevo gasto general';$('save').textContent='Enviar a aprobación';
    $('formFields').innerHTML=`<input type="hidden" name="entry_type" value="${withdrawal?'retiro_utilidad':'gasto'}">`+
      ...
      field('Concepto','concept','text','required minlength="3"',withdrawal?'Retiro de utilidad':'')+field('Socio / beneficiario','beneficiary','text','required minlength="3"','')+
      field('CC / NIT','document')+
    ...
    if(withdrawal){const toggle=()=>{$('retailFunding').hidden=$('f_business').value!=='retail';};$('f_business').addEventListener('change',toggle);toggle();}
  }
  ...
  function actionButtons(row){if(row.status==='pendiente_aprobacion')return '<a class="btn" href="aliados-tesoreria.html?vista=gastos">Pendiente en Tesorería</a>';if(row.status==='aprobado'&&retailWithdrawal(row))return '<a class="btn" href="cuenta-corriente.html#retail">Ver instrucciones y validar soportes de tiendas</a>';if(row.status==='aprobado')return `<button class="btn primary" data-action="payment" data-id="${row.id}">Registrar pago</button>`;return '';}
  ...
  async function saveManual(data){let account;try{account=D.destinationAccount(data.get('bank')==='Otro'?data.get('other_bank'):data.get('bank'),data.get('account_type'),data.get('account'));}catch(error){return {error};}
    if(data.get('entry_type')==='retiro_utilidad'&&data.get('business')==='retail'){
      ...
      const result=await sb.rpc('finanzas_preparar_retiro_retail',{p_datos:payload,p_request_id:retailRequest});
  ...
  function download(){const rows=selectedEntries();if(!rows.length){notice('No hay datos en el rango seleccionado.',true);return;}try{if(!window.XLSX || !window.KoraFinancialReports)throw new Error('No se cargó el exportador. Actualiza la página y vuelve a intentar.');const book=window.KoraFinancialReports.workbook(rows,D,window.XLSX);window.XLSX.writeFile(book,`finanzas-${view}-${today()}.xlsx`);notice('Informe Excel generado con el destino bancario registrado.');}catch(error){notice(errorText(error),true);}}
  function paymentOrder(){try{if(!window.KoraFinancialReports)throw new Error('No se cargó el generador de órdenes. Actualiza la página.');window.KoraFinancialReports.showOrder(selectedEntries(),D);}catch(error){notice(errorText(error),true);}}
  ...
  $('exportPaymentOrder').addEventListener('click',paymentOrder);
