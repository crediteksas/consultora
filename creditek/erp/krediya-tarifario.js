(function(root) {
  ...
  function tarifaRows(rows) {
    return [['Código','Referencia','PVP configurado','PAGAMOS antes de inicial','Vigente desde','Vigente hasta'],
      ...rows.map(r=>[r.codigo || '',r.referencia,r.precio_venta,r.pagamos,r.vigente_desde,r.vigente_hasta || ''])];
  }
  function diferenciasRows(rows) {
    return [['Referencia','Comercio','IMEI','Fecha venta','PVP configurado','PVP Krediya','Diferencia PVP','PAGAMOS pactado','Inicial','Giro al aliado','Bonos','Utilidad neta','Impacto neto','Responsables','Estado','Última gestión','Soporte'],
      ...rows.map(r=>{const c=r.contexto||{},g=last(r);return [c.referencia,c.tienda,c.imei,c.fecha,c.pvp_guardado,c.pvp_liquidado,c.impacto_bruto,c.pagamos,c.inicial,Number(c.pagamos)-Number(c.inicial),c.bonos,c.utilidad_neta,c.impacto_neto,'Oscar y Mayte',r.estado,g?.comentario||'',g?.soporte||''];})];
  }
  ...
  function create({sb,money}) {
    let modal;
    const amount=v=>v==null?'No configurado':money(v);
    ...
    async function openTariff() {
      ...
      modal.innerHTML='<header><h2>Tarifario Krediya</h2><button class="btn secondary" data-close>Cerrar</button></header><p>PVP configurado y PAGAMOS pactado. Giro al aliado = PAGAMOS − inicial. Editar aquí no cambia precios en la plataforma de Krediya ni liquidaciones aprobadas.</p><div data-content>Cargando…</div>';
      const dialog=modal;
      ...
      function render(search='') {
        ...
        content.querySelector('[data-download]').onclick=()=>download(tarifaRows(filtered),'Tarifario-Krediya.xlsx','Tarifario');
        content.querySelectorAll('[data-edit]').forEach(b=>b.onclick=()=>edit(rows.find(r=>r.id===b.dataset.edit)));
      }
      function edit(r) {
        content.innerHTML=`<h3>${esc(r.referencia)}</h3><p>PVP actual: ${amount(r.precio_venta)} · PAGAMOS actual: ${amount(r.pagamos)}</p><form class="tariff-form"><label>PVP configurado<input class="control" name="pvp" type="number" min="0.01" step="0.01" required value="${r.precio_venta}"></label><label>PAGAMOS pactado antes de inicial<input class="control" name="pagamos" type="number" min="0.01" step="0.01" required value="${r.pagamos}"></label><label>Aplicar a ventas desde<input class="control" name="desde" type="date" min="${r.vigente_desde}" value="${new Intl.DateTimeFormat('en-CA',{timeZone:'America/Bogota'}).format(new Date())}" required></label><label>Motivo del cambio<textarea class="control" name="motivo" minlength="5" maxlength="2000" required></textarea></label><p>Se conservarán los valores anteriores en auditoría. Los lotes ya calculados no se actualizan solos; los aprobados no cambian.</p><p data-error role="alert"></p><div class="tariff-controls"><button class="btn secondary" type="button" data-back>Volver</button><button class="btn primary" type="submit">Guardar nueva vigencia</button></div></form>`;
        ...
        content.querySelector('form').onsubmit=async e=>{e.preventDefault();const f=e.currentTarget,button=f.querySelector('[type=submit]');button.disabled=true;
          try {const {error}=await sb.rpc('krediya_guardar_tarifa',{p_id:r.id,p_version:r.updated_at,p_pvp:Number(f.elements.pvp.value),p_pagamos:Number(f.elements.pagamos.value),p_desde:f.elements.desde.value,p_motivo:f.elements.motivo.value.trim()});if(error)throw error;await load();}
    ...
    async function report(container,batch) {
      ...
      container.innerHTML=`<section class="krediya-differences"><header><div><h3>Informe de diferencias de PVP</h3><p>Krediya · Corte ${esc(batch.fecha_corte)} · Responsables: Oscar y Mayte</p></div><button class="btn primary" data-export>Descargar informe Excel</button></header><p>Estas tareas no bloquean la aprobación ni el pago autorizado. Los valores corresponden al cálculo guardado, no al tarifario actual.</p><div class="difference-summary"><strong>${rows.length} diferencias</strong><span>${rows.filter(r=>r.estado!=='resuelta').length} por gestionar</span><span>Impacto neto cuantificado: ${amount(rows.reduce((n,r)=>n+Number(r.contexto.impacto_neto||0),0))}</span></div>${rows.length?'':'<p>Sin diferencias registradas. Si el lote está sin calcular, genera la liquidación para obtener el informe.</p>'}${rows.map(r=>{const c=r.contexto,g=last(r);return `<article class="difference-card"><header><div><h4>${esc(c.referencia)}</h4><p>${esc(c.tienda)} · IMEI ${esc(c.imei)} · Venta ${esc(c.fecha)}</p></div><span>${esc(r.estado.replace('_',' '))}</span></header><dl>${[['PVP configurado',c.pvp_guardado],['PVP Krediya liquidado',c.pvp_liquidado],['Diferencia PVP',c.impacto_bruto],['PAGAMOS pactado',c.pagamos],['Bonos',c.bonos],['Utilidad neta',c.utilidad_neta],['Impacto neto',c.impacto_neto]].map(([label,value])=>`<div><dt>${label}</dt><dd>${amount(value)}</dd></div>`).join('')}</dl>${c.pvp_guardado==null?'<p>Falta PVP de referencia para comparar; el cálculo usa el PVP recibido y respeta PAGAMOS.</p>':''}${g?`<p>Última gestión: ${esc(g.comentario)} · ${esc(g.autor_nombre)}</p>`:''}<details><summary>Gestionar / ver historial</summary>${(r.krediya_diferencias_gestiones||[]).map(g=>`<p>${esc(g.created_at)} · ${esc(g.autor_nombre)} · ${esc(g.estado)}<br>${esc(g.comentario)}<br>${esc(g.soporte||'')}</p>`).join('')}<form data-followup="${esc(r.operation_id)}" class="tariff-form"><label>Estado<select class="control" name="estado"><option value="en_gestion">En gestión</option><option value="resuelta">Resuelta</option></select></label><label>Instrucción o gestión para Oscar y Mayte<textarea class="control" name="comentario" minlength="5" maxlength="4000" required></textarea></label><label>Soporte (obligatorio al resolver)<input class="control" name="soporte" maxlength="2000"></label><p data-error role="alert"></p><button class="btn primary" type="submit">Guardar seguimiento</button></form></details></article>`;}).join('')}</section>`;
      container.querySelector('[data-export]').onclick=()=>download(diferenciasRows(rows),`Diferencias-Krediya-${batch.fecha_corte}.xlsx`,'Diferencias PVP');
  ...
  const api={create,tarifaRows,diferenciasRows,last};
