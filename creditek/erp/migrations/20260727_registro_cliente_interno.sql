...
begin
  ...
  if not found or v_perfil.rol not in ('gerencia','auditoria','admin_tienda','asesor') then
    raise exception 'No autorizado para registrar clientes';
  end if;
  if v_perfil.rol in ('admin_tienda','asesor')
     and v_perfil.tienda_codigo is distinct from p_origen_codigo then
    raise exception 'No autorizado para registrar en otra tienda';
  end if;
  ...
  if exists (select 1 from public.clientes where cedula = p_cedula) then
    raise exception 'Ya existe un cliente con esta cédula';
  end if;
  if exists (select 1 from public.clientes where celular = p_celular) then
    raise exception 'Ya existe un cliente con este celular';
  end if;
  ...
  insert into public.clientes(
    ...
    btrim(p_direccion), p_origen_codigo, 'registro_interno', true,
  ...
  values (
    ...
    jsonb_build_object('origen_codigo', p_origen_codigo, 'creado_por', auth.uid())
  );
