...
(function () {
  ...
  const KORA_VERSION = '3.3.2';
  const KORA_DISPLAY_VERSION = 'KORA v3.3.2';
...
@media (max-width: 1023px) {
  ...
  const MODULOS = [
    ...
    { titulo: 'CREDITEK B2B', b2b: true, icono: '🏭', lucide: 'warehouse', description: 'Compras, proveedores, inventario central y resultado del negocio B2B.', items: [
      ...
      { label: 'Proveedores y cartera', href: 'proveedores.html', lucide: 'hand-coins', description: 'Administra proveedores, facturas, vencimientos, pagos y saldos pendientes.', roles: ['gerencia', 'auditoria'] },
...
@media (max-width: 640px) {
  ...
  function installKoraAssets() {
    ...
    if (!document.getElementById('koraResponsiveStyles')) {
      ...
      link.rel = 'stylesheet';
      link.href = '/design-system/components/kora-responsive.css?v=1.0.8';
      document.head.appendChild(link);
    }
    if (!document.getElementById('koraResponsive')) {
      const script = document.createElement('script');
      script.id = 'koraResponsive';
      script.src = '/creditek/erp/kora-responsive.js?v=1.0.2';
      script.defer = true;
    ...
    if (!document.getElementById('koraReportExport')) {
      ...
      reports.src = '/creditek/erp/kora-report-export.js?v=1.2.0';
  ...
  function mountKoraShell({ root, profile, stores = [], modules = MODULOS, activeItem, onLogout, supabaseClient, productName }) {
    ...
    <div class="kora-sidebar__footer">
      ...
      <button class="ghost kora-nav-text" type="button" data-kora-version style="display:block;width:100%;padding:4px 16px 10px;text-align:left;font-size:11px;line-height:1.45;opacity:.78">
        <span style="display:block">KORA ERP v3.3.2</span><span style="display:block">Build: <span data-kora-build>—</span></span><span style="display:block">Ambiente: Producción</span>
      </button>
