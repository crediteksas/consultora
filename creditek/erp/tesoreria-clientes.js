(function (root, factory) {
  ...
  function directory(origins, beneficiaries, accounts, sites = [], clients = []) {
    return origins.filter(o => o.tipo === 'aliado' && o.activo).map(origin => {
      ...
      return { origin:{...origin,ciudad:origin.ciudad || site?.ciudad}, beneficiary, account, site, client: clients.find(c=>c.id===site?.aliado_id), conflict: holders.length > 1,
  ...
  function create({sb}) {
    ...
    async function refresh() {
      const result = await Promise.all([
        ...
        all('aliados','id,razon_social,identificacion,propietario,contacto,telefono,email,observacion,revision','id')
      ]);
    ...
    function renderList() {
      ...
      $('#clientList').innerHTML = visible.map(r => `<article class="tc-row">
        <div><h3>${esc(r.origin.nombre)}</h3><p>${esc(r.origin.ciudad || 'Ciudad no registrada')}</p><small>${esc(r.client?.contacto || r.origin.codigo)}${r.client?.telefono ? ` · ${esc(r.client.telefono)}` : ''}</small></div>
    ...
    function setPanel(panel) {
      $('#clientProfile').hidden=panel!=='profile'; $('#clientBankPanel').hidden=panel!=='bank';
    ...
    function openBeneficiary(id) {
      ...
      $('#clientHolder').parentElement.hidden=true;$('#clientProfileTab').hidden=true;
    ...
    async function mount(element) {
      ...
      if (!$('#clientList')) {
        ...
        <dialog id="clientDialog" class="tc-dialog" aria-labelledby="clientEditorTitle">
          ...
          <nav class="tc-tabs" aria-label="Secciones de la ficha"><button id="clientProfileTab" class="btn secondary" type="button">Datos del cliente</button><button id="clientBankTab" class="btn secondary" type="button">Cuenta de pago</button></nav>
          ...
          </div><p class="tc-help">El cliente y el titular de la cuenta pueden ser distintos. No copies la identificación del titular como identificación legal del comercio.</p>
