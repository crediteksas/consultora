(function(root){
  ...
  function create({sb,profile,domain,onSummary=()=>{}}){
    ...
    async function submit(event){event.preventDefault();const form=event.target.closest('[data-decision]');if(!form||busy)return;const row=rows.find(r=>r.id===form.dataset.decision);if(!canDecide(profile,row||{}))return;
      ...
      try{const result=await sb.rpc('finanzas_decidir_movimiento',{p_id:row.id,p_decision:decision,p_amount:decision==='aprobado'?amount:null,p_note:form.elements.note.value.trim()||null});if(result.error)throw result.error;await load();message(decision==='aprobado'&&row.retail_funding?'Retiro aprobado: instrucciones emitidas a las tiendas. La caja se descontará al validar sus soportes.':decision==='aprobado'?'Gasto aprobado. No se ha registrado ningún pago ni reserva bancaria.':'Gasto rechazado.');}
