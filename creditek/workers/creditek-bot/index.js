...
async function reservarHandoff(supabaseUrl2, serviceKey, input, fetcher = fetch) {
  ...
  if (!response.ok && response.status !== 409) {
    ...
    throw new Error(`Supabase outbox respondi\xF3 ${response.status}`);
  }
  let rows = [];
...
var MSG = {
  OPTIN: "Para atenderte, \xBFautorizas a Creditek a usar los datos de este chat? Tambi\xE9n puedes aceptar promociones por WhatsApp; son opcionales y puedes cancelarlas escribiendo SALIR. Pol\xEDtica: https://aura.crediteksas.com/creditek/legal/",
...
function botonesConsentimientoUnico() {
  return [
    ...
    { id: "consent_service", title: "Solo atenci\xF3n" },
    { id: "consent_none", title: "No autorizo" }
  ];
}
__name(botonesConsentimientoUnico, "botonesConsentimientoUnico");
var index_default = {
  ...
  async fetch(request, env2) {
    ...
    if (url.pathname === "/api/notificar-asesor" && request.method === "POST") {
      ...
      const conv = {
        ...
        ciudad: cliente.ciudad_normalizada || cliente.ciudad || tienda.ciudad,
        modalidad: cliente.modalidad || "credito",
        producto_interes: cliente.producto_interes,
        tienda_id: tienda.id,
        tienda_nombre: tienda.nombre,
        tienda_nombre_comercial: tienda.nombre_comercial,
        tienda_contacto: tienda.contacto,
      ...
      if (!reserva.permitido) {
        ...
        return new Response(JSON.stringify({ error: "Notificaci\xF3n en revisi\xF3n; no se duplic\xF3 el env\xEDo" }), { status: 409, headers: cors });
      }
      try {
      ...
      await actualizarCliente(clienteId, {
        ...
        fecha_transferido_asesor: (/* @__PURE__ */ new Date()).toISOString()
      }, sk);
      ...
      try {
        await enviarMensajeWA(clienteId, avisoCliente, env2.PHONE_NUMBER_ID, env2.WHATSAPP_TOKEN);
...
Gu\xE1rdalo en favoritos \u{1F60A} \xBFNecesitas algo m\xE1s?`;
      ...
      const MAPA_BOTON_RAPIDO = {
        optin_si: "acepto",
        optin_no: "no, gracias",
        consent_both: "autorizo datos y promociones",
        consent_service: "autorizo solo atencion",
        consent_none: "no autorizo",
        marketing_si: "acepto",
    ...
    case "DATOS_MIN": {
      if (!conv.nombre || conv.modalidad === "credito" && !conv.cedula) {
        try {
          ...
          if (existente) {
            conv.nombre ||= existente.nombre || void 0;
            conv.cedula ||= existente.cedula || void 0;
            conv.celular ||= existente.telefono_contacto || clienteId;
            conv.tienda_id ||= existente.tienda_id || void 0;
            conv.ciudad ||= existente.ciudad_normalizada || existente.ciudad || void 0;
          }
        } catch {
