...
export class PayJoyAdapterReal extends PlatformAdapterReal {
  ...
  normalize(row) { return normalizeLiquidationOperation({ ...row,external_id:row.external_id ?? row.device,monto_credito:row.monto_credito ?? row['owed by PayJoy'] }, this.platform); }
}
export class AloAdapterReal extends PlatformAdapterReal {
  constructor() { super('alo'); }
  normalize(row) { return normalizeLiquidationOperation({ ...row,external_id:row.external_id ?? row.CONTRATO2 ?? row.Contrato,monto_credito:row.monto_credito ?? row.MONTO_CREDITO ?? row['Monto Credito'] }, this.platform); }
}
export class AddiAdapterReal extends PlatformAdapterReal {
  constructor() { super('addi'); }
  normalize(row) { return normalizeLiquidationOperation({ ...row,external_id:row.external_id ?? row.loan_id,monto_credito:row.monto_credito ?? row.approved_amount },this.platform); }
}
