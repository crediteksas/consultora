...
import {safeSearch,safeCreate,safePayJoyCheck} from './nova-sandbox-service.mjs';
...
createServer(async(req,res)=>{try{
  ...
  if(requestUrl.pathname==='/api/nova/search'&&req.method==='GET'){const profile=safeSearch(requestUrl.searchParams.get('query')||'');res.writeHead(200,{'content-type':'application/json','cache-control':'no-store'});return res.end(JSON.stringify({environment:'aura2-nova-sandbox',profile}));}
  if(requestUrl.pathname==='/api/nova/payjoy-check'&&req.method==='POST'){let body='';for await(const chunk of req)body+=chunk;const input=JSON.parse(body||'{}');const check=await safePayJoyCheck(String(input.customer_id||''));res.writeHead(check?200:404,{'content-type':'application/json','cache-control':'no-store'});return res.end(JSON.stringify({environment:'aura2-nova-sandbox',check}));}
  if(requestUrl.pathname==='/api/nova/customers'&&req.method==='POST'){let body='';for await(const chunk of req)body+=chunk;const profile=safeCreate(JSON.parse(body||'{}'));res.writeHead(201,{'content-type':'application/json','cache-control':'no-store'});return res.end(JSON.stringify({environment:'aura2-nova-sandbox',profile}));}
...
}catch(error){res.writeHead(404,{'content-type':'application/json'});res.end(JSON.stringify({error:'sandbox_resource_unavailable'}));}}).listen(port,'127.0.0.1',()=>console.log(`AURA sandbox local: http://127.0.0.1:${port}/`));
