...
const ROUTES = new Map([
  ...
  ['/api/sofia/seguimiento-asesor', { method: 'POST', upstream: '/api/seguimiento-asesor' }],
  ['/api/sofia/reintentar-handoff', { method: 'POST', upstream: '/api/reintentar-handoff' }],
]);
...
export async function handleAuraSofiaProxy(request, env, fetcher = fetch) {
  ...
  if (request.method !== route.method) return reply({ ok: false, error: 'Método no permitido' }, 405);

  let body;
  if (request.method === 'POST') {
    const declaredLength = Number(request.headers.get('content-length') || 0);
    if (declaredLength > MAX_BODY_BYTES) return reply({ ok: false, error: 'Solicitud demasiado grande' }, 413);
    body = await request.text();
    if (body.length > MAX_BODY_BYTES) return reply({ ok: false, error: 'Solicitud demasiado grande' }, 413);
  }

  const base = String(env.SOFIA_WORKER_URL || DEFAULT_SOFIA_WORKER_URL).replace(/\/$/, '');
  const authorization = request.headers.get('authorization') || '';
  const upstreamAuth = env.WORKER_SHARED_SECRET
