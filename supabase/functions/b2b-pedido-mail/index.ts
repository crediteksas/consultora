...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  const report=await rpc('kora_claim_b2b_pedido_mail',{p_id:id,p_token:token});
  ...
  const saved=await rpc('kora_finish_b2b_pedido_mail',{p_id:id,p_token:token,p_outcome:result.outcome,p_message_id:result.messageId||null});
