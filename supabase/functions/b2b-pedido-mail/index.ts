...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  const report=await rpc(kind==='cierre'?'kora_claim_b2b_cierre_mail':'kora_claim_b2b_pedido_mail',{p_id:id,p_token:token});
  ...
  const saved=await rpc(kind==='cierre'?'kora_finish_b2b_cierre_mail':'kora_finish_b2b_pedido_mail',{p_id:id,p_token:token,p_outcome:result.outcome,p_message_id:result.messageId||null});
