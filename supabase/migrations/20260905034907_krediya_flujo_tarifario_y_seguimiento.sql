...
create function public.krediya_exigir_cuenta_al_pagar()
...
create trigger zz_krediya_cuenta_al_pagar before insert or update of estado,bank_account_id on public.payment_orders
 for each row execute function public.krediya_exigir_cuenta_al_pagar();
revoke all on function public.krediya_exigir_cuenta_al_pagar() from public,anon,authenticated;
...
create or replace function public.krediya_guardar_tarifa(
...
declare l public.liquidations%rowtype; o public.liquidation_operations%rowtype;
 ...
 precio numeric; pactado numeric; pago numeric; bonos numeric; bruta numeric; provision numeric; neta numeric;
begin
 if auth.uid() is null or not public.tiene_capacidad_aliados('revisor') then raise exception 'No autorizado'; end if;
 ...
 and exists(select 1 from public.liquidation_operations o where o.id=i.operation_id and o.reconocida);
 ...
 for o in select * from public.liquidation_operations where liquidation_id=p_id and reconocida order by operation_at,id for update loop
  ...
  bruta:=round(precio-pactado-bonos,2); provision:=round(bruta*0.28,2); neta:=bruta-provision;
  snap:=c||jsonb_build_object('motor','krediya_v2','pvp_liquidado',precio,'pagamos',pactado,'bonos',bonos,'utilidad_bruta',bruta,
   ...
   'impacto_neto',neta-((c->>'pvp_guardado')::numeric-pactado-bonos-round(((c->>'pvp_guardado')::numeric-pactado-bonos)*0.28,2)));
  ...
  insert into public.liquidation_calculations(liquidation_id,operation_id,policy_snapshot,pagamos,pago_aliado,total_bonos,utilidad_creditek,explanation)
  values(p_id,o.id,snap,pactado,pago,bonos,neta,snap||jsonb_build_object('valor_comercial',precio,'pago_neto',pago));
