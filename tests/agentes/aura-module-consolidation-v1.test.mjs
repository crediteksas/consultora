...
test('NOVA conserva cinco rutas actuales sin apropiarse de rutas Cartera',()=>{assert.deepEqual(NOVA_ROUTES,['search','profile','sales','validations','portfolio']);assert.equal(NOVA_ROUTES.some(route=>CARTERA_ROUTES.includes(route)),false);});
...
test('router futuro no transporta mensajes y enruta por contrato',()=>{const router=new AuraChannelRouter();assert.equal(router.route({line:'NEW_AURA',user_type:'RETAIL',intent:'AUTHORIZATION'}),'NOVA');assert.equal(router.route({line:'NEW_AURA',user_type:'CUSTOMER',intent:'PAYMENT'}),'CARTERA');assert.equal(router.route({line:'SOFIA_CURRENT',intent:'PAYMENT'}),'SOFIA');assert.equal(router.route({line:'NEW_AURA',user_type:'UNKNOWN',intent:'UNKNOWN'}),'HUMANO');});
