...
test('las 42 pantallas del shell declaran un viewport adaptable', async () => {
...
test('el shell carga una única capa responsive transversal', async () => {
  ...
  assert.match(shell, /kora-responsive\.css\?v=1\.0\.4/);
  assert.match(shell, /koraResponsive/);
  assert.match(shell, /kora-responsive\.js\?v=1\.0\.1/);
});
