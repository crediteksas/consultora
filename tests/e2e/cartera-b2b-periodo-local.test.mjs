...
test('cartera completa: período, detalle, errores, exportación y diseño sin scroll lateral',async()=>{
 ...
 try{
  ...
  for(const width of [390,768,1280]){
   ...
   await page.screenshot({path:`/tmp/kora-cartera-b2b-${width}.png`,fullPage:true});
  }
  ...
  assert.doesNotMatch(await page.locator('#dRows').textContent(),/Fuera de período|Saldo previo/);
  ...
  await page.evaluate(()=>window.failTable='cuenta_corriente');await page.locator('#actualizar').click();
