...
test('dashboard y reporte muestran puente de utilidad y todas las financieras sin errores',async()=>{
 ...
 await page.addInitScript(()=>{
  ...
  window.creditekSidebar={perfil:{rol:'gerencia',activo:true},sb:{from(table){const q=new Proxy({}, {get(_,key){if(key==='then')return ok=>Promise.resolve({data:data[table]||[]}).then(ok);return ()=>q;}});return q;}}};
 });
