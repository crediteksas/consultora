...
test('Addi usa tarifa fija 7,5%, IVA 19% sobre la tarifa y pago a 15 días', () => {
  ...
  assert.deepEqual(addi.calcular(90000), {
    credito: 90000, tarifa: 6750, ivaTarifa: 1282.5, netoEstimado: 81967.5, diasPago: 15,
  });
  assert.deepEqual(addi.calcular(258800), {
    credito: 258800, tarifa: 19410, ivaTarifa: 3687.9, netoEstimado: 235702.1, diasPago: 15,
  });
  assert.deepEqual(addi.calcular(996200), {
    credito: 996200, tarifa: 74715, ivaTarifa: 14195.85, netoEstimado: 907289.15, diasPago: 15,
  });
});
