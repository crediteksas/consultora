-- Prueba de integración multilocal con datos sintéticos. No aprueba ni paga.
-- Ejecutar completa: todo se revierte con ROLLBACK.
begin;
...
begin
  ...
  insert into public.origenes(codigo,nombre,tipo,activo,ejecutivo_id) values
    ...
    (prefix||'A2',prefix||' SEGUNDO LOCAL','aliado',true,ejecutivo),
  ...
  update public.aliados set payment_beneficiary_id=aliado where id=(select aliado_id from public.aliados_sedes where origen_codigo=prefix||'A');
  perform set_config('request.jwt.claim.sub',auditoria::text,true);
  perform set_config('request.jwt.claims',jsonb_build_object('sub',auditoria,'role','authenticated')::text,true);
  perform public.tesoreria_vincular_local_cliente(prefix||'A2',
    (select aliado_id from public.aliados_sedes where origen_codigo=prefix||'A2'),
    (select aliado_id from public.aliados_sedes where origen_codigo=prefix||'A'));
  assert public.aliados_beneficiario_de_comercio(prefix||'A2')=aliado, 'Segundo local comparte titular';
  perform set_config('request.jwt.claim.sub',gerencia::text,true);
  perform set_config('request.jwt.claims',jsonb_build_object('sub',gerencia,'role','authenticated')::text,true);
  ...
  values
    ...
    (perdida,lote,'krediya',gen_random_uuid()::text,prefix||'P','2026-08-24 16:00Z',prefix||' SEGUNDO LOCAL',prefix||'A2','aliado',ejecutivo,
...
do $$
begin
 assert (select v from kv2_before where k='otros_lotes')=(select coalesce(jsonb_agg(to_jsonb(l) order by id),'[]') from public.liquidations l where plataforma<>'krediya'), 'Otras plataformas intactas';
 assert (select v from kv2_before where k='otras_operaciones')=(select coalesce(jsonb_agg(to_jsonb(o) order by id),'[]') from public.liquidation_operations o where plataforma<>'krediya'), 'Ventas de otras plataformas intactas';
 assert (select v from kv2_before where k='reglas_bonos')=(select coalesce(jsonb_agg(to_jsonb(r) order by id),'[]') from public.krediya_bonus_rules r), 'Bonos intactos';
end $$;
rollback;
select 'PASS: dos locales, mismo cliente, una orden agrupada; permisos y cálculo; ROLLBACK completo' resultado;
