...
test('un item sin costo conserva su conteo físico aunque bloquee el valor', () => {
  const resumen = domain().resumir([
    { tipoProducto: 'serializado', unidad_id: 'u1', imei: '111', costo: 0 },
    { tipoProducto: 'cantidad', producto_id: 'p1', nombreProducto: 'Cable', cantidad: 2, costo: null },
  ]);

  assert.equal(resumen.unidades, 3);
  assert.equal(resumen.celulares, 1);
  assert.equal(resumen.accesorios, 2);
  assert.equal(resumen.valorTotal, 0);
  assert.equal(resumen.novedades.length, 2);
});
