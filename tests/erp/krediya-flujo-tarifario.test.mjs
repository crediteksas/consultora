...
const engine=sql.slice(sql.indexOf('create function public.krediya_calcular_y_enviar_aprobacion'),sql.indexOf('create or replace function public.aliados_cambiar_estado'));
...
test('motor SQL respeta Pagamos, resta inicial una vez y aplica provisión tras bonos',()=>{
 ...
 assert.match(engine,/bruta:=round\(precio-pactado-bonos,2\); provision:=round\(bruta\*0.28,2\); neta:=bruta-provision/);
