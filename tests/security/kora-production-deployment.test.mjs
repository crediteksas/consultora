...
test('KORA v3.3.2 queda registrada en configuración, shell y documentación', async () => {
  ...
  assert.equal(JSON.parse(version).version, '3.3.2');
  assert.match(sidebar, /KORA v3\.3\.2/);
  assert.match(sidebar, /Acerca de KORA/);
  assert.match(sidebar, /KORA ERP v3\.3\.2/);
  assert.match(sidebar, /Versión (?:no )?verificada/);
  assert.match(documentation, /KORA v3\.3\.2/);
...
test('el endpoint runtime entrega la release activa y confirma coincidencia', async () => {
  ...
  const response = await worker.fetch(new Request('https://registro.crediteksas.com/kora-build-manifest.json'), {
    ASSETS: { fetch: async () => Response.json({ displayVersion: 'KORA v3.3', commit: 'abc1234' }) },
