...
test('todas las páginas ERP versionan el shell y su guard con el mismo identificador', async () => {
  ...
  assert.equal(shellPages, 44, 'debe validar todas las páginas ERP que montan el shell');
});
...
test('el build conserva la versión única vigente del shell', async () => {
  ...
  assert.match(buildScript, /const KORA_SHELL_ASSET_VERSION = '2\.0\.32'/);
  assert.match(buildScript, /const KORA_ACCESS_CONTROL_ASSET_VERSION = '2\.0\.32'/);
